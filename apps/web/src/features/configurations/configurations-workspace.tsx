...
import { Database, FileCode2, Layers, Radio, Search, ShieldCheck } from "lucide-react";

import { MetricStrip, Panel, StatusLabel, WorkspaceTitle } from "@/components/soc/flagship-ui";
import type { AutonomyPolicy, IntegrationRecord, Metrics } from "@/lib/api";
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] bg-background pb-6">
      ...
      <div className="grid gap-4 p-4 lg:grid-cols-2 lg:p-5">
        ...
        <Panel title="Detection engine" eyebrow="Rules">
          <div className="grid grid-cols-2 gap-2 p-4">
            ...
            <div className="soc-inset col-span-2 p-3"><p className="soc-label mb-1.5 flex items-center gap-1"><Radio className="size-3" /> Coverage</p><div className="flex flex-wrap gap-1">{metrics.rules.mitre_techniques.slice(0, 12).map((t) => <span key={t} className="rounded bg-primary/10 px-1.5 py-0.5 font-mono text-[10px] text-primary">{t}</span>)}</div></div>
          </div>
        </Panel>
