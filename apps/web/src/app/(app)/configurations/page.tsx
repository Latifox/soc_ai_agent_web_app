...
import { getAutonomyPolicies, getIntegrations, getMetrics, getSettings, getWhoami } from "@/lib/api";
import type { WorkspaceMetric } from "@/lib/workspace-data";

export const metadata: Metadata = { title: "Configurations" };

export default async function ConfigurationsPage() {
  const [metrics, integrations, policies, who, settings] = await Promise.all([getMetrics(), getIntegrations(), getAutonomyPolicies(), getWhoami(), getSettings()]);
  ...
  return <ConfigurationsWorkspace metrics={metrics} integrations={integrations} policies={policies} tenantId={who.tenant_id} strip={strip} settings={settings} />;
}
