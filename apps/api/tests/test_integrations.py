...
def test_add_opensearch_connector_registers_agent_access(client, make_token):
    headers = _admin(make_token)
    body = {
        "provider": "opensearch",
        "name": "OpenSearch",
        "config": {"url": "http://localhost:9200", "agent_access": True},
    }
    ...
    assert connector_registry.config_for("acme", "opensearch") is not None
...
def test_revoke_agent_access_hides_connector_from_crew(client, make_token):
    ...
    created = client.post(
        "/api/v1/integrations",
        json={"provider": "opensearch", "name": "OS2", "config": {"url": "http://localhost:9200", "agent_access": True}},
        headers=headers,
    ).json()
    assert connector_registry.config_for("acme", "opensearch") is not None
    client.patch(
        f"/api/v1/integrations/{created['id']}",
        json={"config": {"url": "http://localhost:9200", "agent_access": False}},
        headers=headers,
    )
    assert connector_registry.config_for("acme", "opensearch") is None
